...
cv_folds <- crossv_mc(birthweight, 100) # 10-fold cross-validation

# Define a function to calculate RMSE
calc_rmse <- function(model, data) {
  sqrt(mean((model$resid)^2))
}

# Apply models to each fold using purrr
cv_results <- cv_folds |>
  mutate(
...
average_rmse1 <- mean(cv_results$rmse1)
average_rmse2 <- mean(cv_results$rmse2)
```
